...
b_LHS=LHS_Call(2.7207e-02-(0.7*2.7207e-02), b, 2.7207e-02+(0.7*2.7207e-02), 0.01,runs,'unif'); 
mu_LHS=LHS_Call(1/43.42676617-(0.7*1/43.42676617), mu, 1/43.42676617+(0.7*1/43.42676617), 0.01,runs,'unif'); 
d_LHS=LHS_Call(0.01-(0.7*0.01), d, 0.001+(0.7*0.01), 0.01 ,runs,'unif'); 
beta_LHS=LHS_Call(0.6503-(0.7*0.6503), beta, 0.6503+(0.7*0.6503), 0.01 ,runs,'unif'); 
epsilon_LHS=LHS_Call(0.16-(0.7*0.16), epsilon, 0.16+(0.7*0.16), 0.01 ,runs,'unif'); 
k_LHS=LHS_Call(4-(0.7*4), k, 4+(0.7*4), 0.01 ,runs,'unif'); 
r1_LHS=LHS_Call(1.5381-(0.7*1.5381), r1, 1.5381+(0.7*1.5381), 0.01 ,runs,'unif'); 
r2_LHS=LHS_Call(0.0075, r2, 0.0425, 0.01 ,runs,'unif');
r3_LHS=LHS_Call(0.003, r3, 0.017, 0.01 ,runs,'unif'); 
w_LHS=LHS_Call(0.015, w, 0.085, 0.01 ,runs,'unif'); 
p_LHS=LHS_Call(0.05-(0.7*0.05), p, 0.05+(0.7*0.05), 0.01 ,runs,'unif'); 
q_LHS=LHS_Call(0.1-(0.7*0.1), q, 0.1+(0.7*0.1), 0.01 ,runs,'unif'); 
...
LHSmatrix=[b_LHS mu_LHS d_LHS beta_LHS epsilon_LHS k_LHS r1_LHS r2_LHS r3_LHS w_LHS p_LHS q_LHS v_LHS dummy_LHS];
...
for x=1:runs
    ...
    disp(['x : ',num2str(x),', LHSmatrix : [',...
        ...
        num2str(LHSmatrix(x,10)),',',num2str(LHSmatrix(x,11)),',',num2str(LHSmatrix(x,12)),',',num2str(LHSmatrix(x,13)),',',num2str(LHSmatrix(x,14)),']'])
    [t,y]=ode45(@(t,y)f(t,y,LHSmatrix(x,1),LHSmatrix(x,2),...
        LHSmatrix(x,3),LHSmatrix(x,4),LHSmatrix(x,5),LHSmatrix(x,6),...
        LHSmatrix(x,7),LHSmatrix(x,8),LHSmatrix(x,9),...
        LHSmatrix(x,10),LHSmatrix(x,11),LHSmatrix(x,12),LHSmatrix(x,13)),tspan,y0,[]);
    A=[t y];
    
    S_lhs(:,x)=A(time_points+1,2);
    L_lhs(:,x)=A(time_points+1,3);
    I_lhs(:,x)=A(time_points+1,4);
    C_lhs(:,x)=A(time_points+1,5);
